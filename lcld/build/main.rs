use std::{io::{Read,Write, ErrorKind},path::PathBuf};
...
fn main() -> Result<(),std::io::Error>{
    ...
    let cfg = toml::from_str::<config::Config>(&str).map_err(|e|std::io::Error::new(ErrorKind::InvalidData, e))?;

    let host = cfg.target.host_target.map(|t|t.0).unwrap_or_else(|| 
        target_tuples::Target::parse(&std::env::var("TARGET").unwrap())
    );

    let target = cfg.target.default_target.map(|t|t.0).unwrap_or_else(|| 
        host.clone()
    );
    ...
    println!("cargo:rustc-env=config_targ_generated={}",targ_output.display());
