use std::{io::{Error as IOError, ErrorKind}, path::{PathBuf,Path}};
...
fn find_library<P: AsRef<Path>>(lib: &str, search_dirs: impl IntoIterator<Item=P>,info: &TargetInfo, status: &InputStatus) -> std::io::Result<PathBuf>{
    ...
    match (status.prefer_mode,staticlib_paths.is_empty(),dynlib_paths.is_empty()){
        (LibraryType::Dynamic,_,false) | (_,true,false) => Ok(dynlib_paths.into_iter().next().unwrap()),
        (LibraryType::Static,false,_) | (_,false,true) => Ok(staticlib_paths.into_iter().next().unwrap()),
        (_,true,true) => Err(std::io::Error::new(ErrorKind::NotFound,format!("Library {} not found",lib)))
    }
}


fn print_input_and_ident<'a,R,P: AsRef<Path>>(p: &InputSet, search_dirs: &'a R,info: &TargetInfo, status: &mut InputStatus) -> std::io::Result<()> where &'a R: IntoIterator<Item=P>{
    match p{
...
pub fn main() -> Result<(), IOError> {
    ...
    let mut search_dirs = Vec::new();
     cfg.search_paths.iter().map(|&r|r).map(Path::new)
